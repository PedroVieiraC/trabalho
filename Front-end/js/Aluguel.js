...
// Função para buscar aluguéis encerrados
async function listarAlugueisEncerrados(cpfCliente) {
  try {
    ...
    const data = await response.json(); // Supondo que a resposta seja um array ou um objeto com uma propriedade "data"
    
    // Verifica se a resposta é um array ou um objeto com uma propriedade "data"
    const alugueisEncerrados = Array.isArray(data) ? data : data.data;

    if (!Array.isArray(alugueisEncerrados)) {
      throw new Error('A resposta da API não é um array.');
    }

    exibirAlugueisEncerrados(alugueisEncerrados); // Exibe os aluguéis encerrados
  } catch (error) {
...
function filtrarAlugueis() {
  ...
  if (filtroStatus === 'todos') {
    ...
    listarAlugueisEncerrados(cpfCliente); // Lista aluguéis encerrados
